...
- **Session revocation (MAIR-264)** — `jwt_manager::generate_session_jwt` puts an optional `sid` (session id) claim in the token (`generate_jwt` = no `sid`). `check_jwt_validity` (hence `JwtMiddleware` and `AdminMiddleware`) refuses a token whose `sid` has a `revoked:<sid>` key in Redis (`401 Revoked token`), and fails closed with `503` when Redis cannot be checked; tokens without `sid` never touch Redis. `jwt_manager::revoke_session(redis, sid, ttl)` writes the key (`SET … EX`); Core_API calls it on every revocation path. `revoked:<sid>` are the only keys not prefixed with the role, and only these two functions reach them (crate-private raw methods on `Redis`): the Redis ACL gives every API role read access to `revoked:*` and Core_API write access. Tests: `tests/session_revocation_tests.rs` (own binary, since `middlewares_tests` changes `JWT_TIMEOUT`).
- **`security`** — actix-web middlewares built on `jwt_manager` + `database::query_views`, all expecting `web::Data<AppState>` in app data:
  - `JwtMiddleware` — validates the JWT on every request except the explicit allow-list in `security::is_public_path` (exactly `/`, `/swagger-ui` and `/api-docs` and below, and below `/api/v1/auth/` without dot segments — a path merely containing `auth` is **not** public); on success inserts `AuthenticatedUser` into request extensions.
