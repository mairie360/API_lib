...
Integration tests live in `tests/*.rs` and spin up real Postgres/Redis via `testcontainers` (Docker is required). Most tests share one Postgres container and one seeded dataset via `test_setup::queries_setup::get_shared_db()`, which uses a `tokio::sync::OnceCell` so the container/migrations/seed run once per test binary. The cell stores the setup outcome, failure included: when the setup fails, every test fails at once with that reason instead of starting a new container. Seeded accounts carry an argon2id hash of `SEED_PASSWORD` (`seed_password_hash()`), because `users.password` refuses anything else (`chk_users_password_hashed`). Migrations are applied by running the `ghcr.io/mairie360/liquibase-migrations` image against the `ghcr.io/mairie360/database` container (see `src/test_setup/db_setup.rs`) — pulling these images requires registry access. Postgres is published on a random host port (never the host network / `5432`, so an unrelated local Postgres cannot be picked up instead), and the shared container is removed by an `atexit` hook since the `static` holding it is never dropped.
...
- **`redis`** — `redis_interface::Redis` wraps `deadpool-redis`. Has plain (`get`/`set`/`delete`/`expire`) and `secure_*` variants; the `secure_*` variants are idempotent no-ops when the key already exists/doesn't exist (e.g. `secure_set` skips if the key is already present, `secure_delete`/`secure_expire` skip if absent). `set_ex` / `secure_set_ex` set a TTL atomically with `SET … EX [NX]` (never `SETEX`, which the chart's ACL does not grant). **Every key is prefixed with `<role>:`** (MAIR-267), the role coming from `REDIS_KEY_PREFIX`, else the username of `REDIS_URL`, else `REDIS_USERNAME`; none of them (local Redis without ACL) = no prefix. Callers pass bare keys; `Redis::with_key_prefix` sets it explicitly (tests). Tests against the chart's exact ACL lines: `tests/redis_acl_tests.rs`.
...
- **Session revocation (MAIR-264)** — `jwt_manager::generate_session_jwt` puts an optional `sid` (session id) claim in the token (`generate_jwt` = no `sid`). `check_jwt_validity` and the historical-token branch of `authenticate_token` (hence `JwtMiddleware` and `AdminMiddleware`) refuses a token whose `sid` has a `revoked:<sid>` key in Redis (`401 Revoked token`), and fails closed with `503` when Redis cannot be checked; tokens without `sid` never touch Redis. `jwt_manager::revoke_session(redis, sid, ttl)` writes the key (`SET … EX`); Core_API calls it on every revocation path. Keys are never prefixed by this crate: the Redis ACL gives every API role read access to `revoked:*` and Core_API write access. Tests: `tests/session_revocation_tests.rs` (own binary, since `middlewares_tests` changes `JWT_TIMEOUT`).
- **`security`** — actix-web middlewares built on `jwt_manager` + `database::query_views`, all expecting `web::Data<AppState>` in app data:
  - `JwtMiddleware` — authenticates the bearer token (`authenticate_token`: historical JWT or Keycloak access token) on every request except the explicit allow-list in `security::is_public_path` (exactly `/`, `/swagger-ui` and `/api-docs` and below, and below `/api/v1/auth/` without dot segments — a path merely containing `auth` is **not** public); on success inserts `AuthenticatedUser` into request extensions.
